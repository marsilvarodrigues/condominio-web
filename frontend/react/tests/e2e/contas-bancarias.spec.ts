...
const LANCAMENTOS = [
  { id: 1, data: '2026-06-01', descricao: 'Taxa condominial', valor: 500, saldo: 48820.50, tipo: 'CREDITO', conciliado: true, contaBancariaId: 1 },
  { id: 2, data: '2026-05-30', descricao: 'Pagamento fornecedor', valor: -1200, saldo: 48320.50, tipo: 'DEBITO', conciliado: false, contaBancariaId: 1 },
]
...
test.describe('Contas Bancárias', () => {
  ...
  test('abre dialog de nova conta', async ({ page }) => {
    ...
    await expect(page.getByLabelText(/agência/i)).toBeVisible()
    await expect(page.getByLabelText(/^conta/i)).toBeVisible()
  })
  ...
  test('abre extrato ao clicar em Ver extrato', async ({ page }) => {
    mockGet(page, '**/api/contas-bancarias/**/lancamentos', LANCAMENTOS)
