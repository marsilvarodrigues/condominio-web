...
test.describe('Alterar Senha', () => {
  ...
  test('exibe erro quando senhas não coincidem', async ({ page }) => {
    await page.getByLabel(/senha atual/i).fill('senhaAtual123')
    await page.getByLabel('Nova senha', { exact: true }).fill('NovaSenha@1')
    await page.getByLabel(/confirmar/i).fill('SenhaDiferente@1')
  ...
  test('envia formulário com dados válidos', async ({ page }) => {
    ...
    await page.getByLabel(/senha atual/i).fill('SenhaAtual@1')
    await page.getByLabel('Nova senha', { exact: true }).fill('NovaSenha@1234')
    await page.getByLabel(/confirmar/i).fill('NovaSenha@1234')
    await page.getByRole('button', { name: /salvar|alterar/i }).click()
