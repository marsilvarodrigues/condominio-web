...
/**
 * MapStruct mapper for converting between {@link Proprietario} entities and their DTO
 * representations. Entity creation is handled manually because the concrete sub-type depends on the
 * {@code tipo} discriminator.
 */
@Mapper(
...
public abstract class ProprietarioMapper {
  ...
  /**
   * Converts a {@link Proprietario} entity to its full DTO, including the list of apartments and
   * discriminated type.
   *
  ...
  @Mapping(
      target = "apartamentos",
      source = "proprietario",
      qualifiedByName = "resolveApartamentos")
  ...
  /**
   * Applies the non-null fields from {@code dto} onto the existing {@link Proprietario} entity. The
   * {@code tipo} discriminator, {@code id}, {@code condominio}, and audit timestamps are never
  ...
  /**
   * Extracts the razão social for {@link ProprietarioPessoaJuridica}; returns {@code null}
   * otherwise.
   *
   * @param proprietario source entity
