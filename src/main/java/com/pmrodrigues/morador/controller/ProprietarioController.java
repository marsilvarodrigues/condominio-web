...
/**
 * REST controller exposing CRUD endpoints for proprietários ({@code /proprietarios})
 * and apartment association sub-resources.
 */
@Slf4j
@ApiVersion("1")
@RestController
...
public class ProprietarioController {
    ...
    public ResponseEntity<ApiResponse<ProprietarioDTO>> create(
            @RequestBody @Valid CreateProprietarioDTO dto,
            HttpServletRequest request) {
        log.info("POST /proprietarios - creating proprietario: nome={}, tipo={}", dto.nome(), dto.tipo());
    ...
    /**
     * Updates the mutable fields of an existing proprietario; requires ADMIN role.
    ...
    public ResponseEntity<ApiResponse<ProprietarioDTO>> update(
            @PathVariable Long id,
            @RequestBody UpdateProprietarioDTO dto,
            HttpServletRequest request) {
    ...
    @Timed(value = "proprietario.controller.associarApartamento", description = "Associate proprietario with apartamento")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<ProprietarioDTO>> associarApartamento(
            @PathVariable Long id,
            @RequestParam Long apartamentoId,
            HttpServletRequest request) {
        log.info("POST /proprietarios/{}/apartamentos - associating with apartamento {}", id, apartamentoId);
    ...
    @Timed(value = "proprietario.controller.desassociarApartamento", description = "Dissociate proprietario from apartamento")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @Timed(value = "proprietario.controller.findByApartamento", description = "List proprietarios for apartamento")
