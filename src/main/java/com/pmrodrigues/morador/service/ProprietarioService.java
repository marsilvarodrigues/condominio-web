 ...
 * Extends {@link UserService} to reuse user-creation ceremony (email uniqueness check, activation
 * email dispatch).
 */
@Slf4j
@Service
public class ProprietarioService extends UserService {
  ...
  /**
   * Constructs a {@code ProprietarioService} with all required collaborators.
   *
   ...
   * @param repository repository for {@code Proprietario} persistence
   * @param mapper MapStruct mapper for proprietario DTO conversions
   */
  ...
  public ProprietarioDTO findById(Long id) {
    ...
    var result =
        repository.findById(id).map(mapper::toDTO).orElseThrow(() -> notFound("Proprietario", id));
  ...
  @Transactional(readOnly = true)
  @Timed(
      value = "proprietario.service.findByApartamento",
      description = "Find proprietarios by apartamento")
  public List<ProprietarioDTO> findByApartamento(Long apartamentoId) {
    ...
    var result =
        repository.findByApartamentosId(apartamentoId).stream().map(mapper::toDTO).toList();
    log.info("Found {} proprietarios for apartamento {}", result.size(), apartamentoId);
    return result;
  }

  /**
   * Persists a new Proprietario of the appropriate sub-type, delegating the user-creation ceremony
   * to {@link #persistUser}.
   *
  ...
  /**
   * Updates the mutable fields of an existing proprietario. The tipo discriminator cannot be
  ...
  public ProprietarioDTO update(Long id, UpdateProprietarioDTO dto) {
    ...
    var entity = repository.findById(id).orElseThrow(() -> notFound("Proprietario", id));
  ...
  @Transactional
  @Timed(
      value = "proprietario.service.associarApartamento",
   ...
   * @param apartamentoId apartment primary key
   * @return updated proprietario DTO
   * @throws org.springframework.web.server.ResponseStatusException with 404 if the proprietario is
   *     not found
   */
  @Transactional
  @Timed(
      value = "proprietario.service.desassociarApartamento",
  ...
  public void delete(Long id) {
    ...
    Proprietario entity = repository.findById(id).orElseThrow(() -> notFound("Proprietario", id));
