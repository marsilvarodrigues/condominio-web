...
/**
 * Application service for managing {@code Pessoa} entities, with soft-delete and multi-tenancy
 * support. Extends {@link UserService} to reuse user-creation ceremony (email uniqueness check,
...
public class PessoaService extends UserService {
   ...
   *
   * @param userRepository repository for {@code User} persistence
   * @param mailService service for sending activation emails
   * @param userMapper MapStruct mapper for user DTO conversions
  ...
  public Page<PessoaDTO> filterBy(PessoaFilterDTO filter, Pageable pageable) {
    log.info(
        "Filtering pessoas: nome={}, tipo={}, cpf={}, email={}",
        filter.nome(),
        filter.tipo(),
        filter.cpf(),
        filter.email());
  ...
  /**
   * Persists a new Morador, delegating the user-creation ceremony to {@link #persistUser}. The
   * Morador entity is built by the mapper and enriched with an optional apartment assignment.
   *
  ...
  public PessoaDTO update(Long id, UpdatePessoaDTO dto) {
    ...
    var entity = pessoaRepository.findById(id).orElseThrow(() -> notFound("Pessoa", id));
  ...
  /**
   * Soft-deletes the pessoa with the given id. Refuses deletion when the person is currently
   * assigned to an apartment or has active ownership.
   *
  ...
  public void delete(Long id) {
    ...
    if (entity.getApartamento() != null) {
      log.error(
          "Cannot delete pessoa {}: is morador of apartamento {}",
          id,
    ...
    proprietarioRepository.findByApartamentosId(id).stream()
        .findAny()
        .ifPresent(
            p -> {
              log.error("Cannot delete pessoa {}: has active ownership", id);
              throw new ResponseStatusException(
                  HttpStatus.CONFLICT, "Pessoa tem propriedade ativa em um apartamento");
            });
  ...
  /**
   * Soft-deletes all pessoas belonging to the given condominio. Used by {@code
   * CondominioService.delete()} to cascade the deletion.
   *
   * @param condominioId condominio primary key
   */
  @Transactional
  @Timed(
      value = "pessoa.service.softDeleteByCondominioId",
      description = "Soft delete all pessoas by condominio")
  ...
  /**
   * Returns all active pessoas (moradores) assigned to the given apartment. Used by the billing
   * module to resolve the resident for a charge without crossing module boundaries through a direct
   * repository call.
   *
   * @param apartamentoId apartment primary key
  ...
  public java.util.List<PessoaDTO> listarPorApartamento(Long apartamentoId) {
    ...
    var result =
        pessoaRepository.findByApartamentoId(apartamentoId).stream()
            .map(pessoaMapper::toDTO)
            .toList();
