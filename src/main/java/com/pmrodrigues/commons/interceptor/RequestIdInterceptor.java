...
/**
 * HandlerInterceptor that validates the {@code X-Request-ID} header as a UUID and stores it as a
 * request attribute.
 */
@Slf4j
@Component
...
public class RequestIdInterceptor implements HandlerInterceptor {
  ...
  /**
   * Rejects requests whose {@code X-Request-ID} header is absent or not a valid UUID with a 400
   * response.
   *
   * @return {@code true} if the header is valid and processing should continue; {@code false} if
   *     rejected
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
      throws Exception {
    log.info(
        "Validating X-Request-ID header for request: {} {}",
        request.getMethod(),
    ...
    try {
      ...
      writeBadRequest(
          response, null, "invalid_request_id", "Header X-Request-ID must be a valid UUID");
      return false;
    }
  }

  private void writeBadRequest(
      HttpServletResponse response, UUID requestId, String error, String message) throws Exception {
    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
    response.setContentType(MediaType.APPLICATION_JSON_VALUE);
    objectMapper.writeValue(
        response.getWriter(), ApiResponse.of(requestId, new ErrorResponse(error, message)));
  }
}
