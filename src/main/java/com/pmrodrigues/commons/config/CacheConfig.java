...
public class CacheConfig {
  ...
  public CacheManager cacheManager(
      AppCacheProperties properties, RedisConnectionFactory connectionFactory) {
    return new TwoLevelCacheManager(buildL2Manager(properties, connectionFactory), properties);
  }

  private RedisCacheManager buildL2Manager(
      AppCacheProperties properties, RedisConnectionFactory connectionFactory) {
    var mapper =
        new ObjectMapper()
    ...
    var perCacheConfigs =
        properties.getCaches().keySet().stream()
            .collect(
                Collectors.toMap(
                    name -> name, name -> defaultConfig.entryTtl(properties.l2Ttl(name))));
