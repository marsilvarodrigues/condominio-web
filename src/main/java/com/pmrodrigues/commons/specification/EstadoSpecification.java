...
/**
 * Factory class providing JPA {@link Specification} predicates for querying {@link Estado}
 * entities.
 */
...
public class EstadoSpecification {
  ...
  /**
   * Returns a specification that matches estados whose name contains the given substring
  ...
  /**
   * Returns a specification that matches estados with the given UF abbreviation (case-insensitive
   * exact match), or {@code null} when the filter is absent.
   *
  ...
  public static Specification<Estado> hasUf(String uf) {
    ...
    return (root, query, cb) -> cb.equal(cb.lower(root.get("uf")), uf.toLowerCase());
  }
}
