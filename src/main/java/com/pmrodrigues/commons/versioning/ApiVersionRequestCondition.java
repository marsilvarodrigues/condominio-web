...
/**
 * Custom MVC {@link RequestCondition} that matches requests against the {@code X-API-Version}
 * header.
 *
 * <p>Matching rules:
 *
 * <ul>
 *   <li>Header absent or blank — the condition matches (the controller is a candidate); {@link
 *       #compareTo} selects the highest-version candidate, so the latest version wins.
 *   <li>Header present and equals this condition's version — match.
 *   <li>Header present but different — no match ({@code null} returned).
 * </ul>
 */
public class ApiVersionRequestCondition implements RequestCondition<ApiVersionRequestCondition> {
   ...
   *
   * <p>A missing header causes the condition to return itself so the controller becomes a
   * candidate; {@link #compareTo} then picks the highest-version winner.
   *
  ...
  /**
   * Compares two conditions so that the one with the higher version number wins. A negative return
   * value means {@code this} is preferred over {@code other}.
   *
