...
/**
 * Servlet filter that populates the MDC with correlation, request, and idempotency IDs for
 * structured logging.
 */
@Slf4j
@Component
...
public class MdcContextFilter extends OncePerRequestFilter {
  ...
  /**
   * Populates the MDC, logs request entry/exit with duration, and ensures the MDC is cleared after
   * the response.
   *
  ...
  private void populateMdc(HttpServletRequest request) {
    ...
    MDC.put(
        MDC_CORRELATION_ID, correlationId != null ? correlationId : UUID.randomUUID().toString());
