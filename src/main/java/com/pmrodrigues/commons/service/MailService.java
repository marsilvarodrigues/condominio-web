 ...
 *
 * <p>Templates are Velocity {@code .vm} files loaded from the classpath under
 * {@code templates/email/}. Each template is paired with a {@link Template}
 * implementation whose component names match the {@code $variableName} placeholders
 * in the file.
 *
 * <p>The single public entry point is {@link #sendEmail(String, Template)};
 * {@link #sendHtml(String, String, String)} is available for callers that already hold a
 * pre-rendered body.
 */
@Slf4j
@Service
public class MailService {
    ...
    public void sendEmail(String to, Template template) {
        ...
        try {
            ...
            log.error("Failed to render or send email template={} to={}: {}", template.templatePath(), to, e.getMessage());
            throw e;
        }
    }
