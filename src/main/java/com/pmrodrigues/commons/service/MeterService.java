...
/**
 * Thin wrapper around Micrometer for incrementing tagged error counters without exposing {@link
...
public class MeterService {
  ...
  /**
   * Registers (if not already registered) and increments a counter with {@code error_type} and
   * {@code http_status} tags.
   *
  ...
  public void incrementError(
      String meterName, String errorType, String description, int httpStatus) {
    log.info(
        "Incrementing error counter - meter: {}, errorType: {}, httpStatus: {}",
        meterName,
        errorType,
        httpStatus);
