...
public class BlocoSpecification {
    ...
    /**
     * Returns a specification that matches blocos whose name contains the given substring (case-insensitive), or {@code null} when the filter is absent.
     *
    ...
    public static Specification<Bloco> hasBloco(String bloco) {
        ...
        return (root, query, cb) -> cb.like(cb.lower(root.get("bloco")), "%" + bloco.toLowerCase() + "%");
    }
}
