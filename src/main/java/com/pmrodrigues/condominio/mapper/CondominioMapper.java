...
/**
 * MapStruct mapper for converting between {@link Condominio} entities and their DTO
 * representations.
 */
@Mapper(
...
public abstract class CondominioMapper {
  ...
  /**
   * Creates a new {@link Condominio} entity from a creation payload. The address state is resolved
   * via a JPA reference, avoiding unnecessary database lookups at mapping time.
   *
  ...
  /**
   * Converts a {@link CreateCondominioEnderecoDTO} to the {@link Endereco} embeddable, resolving
   * the estado via a JPA reference instead of a hollow entity.
   */
