...
public class CondominioController {
    ...
    public ResponseEntity<ApiResponse<List<CondominioDTO>>> findAll(
            @ModelAttribute CondominioFilterDTO filter,
    ...
    public ResponseEntity<ApiResponse<CondominioDTO>> findById(@PathVariable Long id, HttpServletRequest request) {
        log.info("GET /condominios/{} - finding condominio by id", id);
        var condominio = condominioService.findById(id)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Condominio not found: " + id));
     ...
     *
     * @param dto creation payload: nome, cnpj (validated), email (validated), endereco with 8-digit cep and numeric estado id
     * @param request current HTTP request used to extract the correlation request ID
    ...
    public ResponseEntity<ApiResponse<CondominioDTO>> create(@RequestBody @Valid CreateCondominioDTO dto,
                                                              HttpServletRequest request) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<CondominioDTO>> update(@PathVariable Long id,
                                                              @RequestBody @Valid CondominioDTO dto,
                                                              HttpServletRequest request) {
        log.info("PUT /condominios/{} - updating condominio", id);
        var updated = condominioService.update(
                new CondominioDTO(id, dto.nome(), dto.cnpj(), dto.email(), dto.endereco(), dto.createdAt(), dto.updatedAt()));
