...
/**
 * Application service for managing apartamentos, with tenant-scoped caching and soft-delete support.
 */
@Slf4j
@Service
...
public class ApartamentoService {
    ...
    @Cacheable(value = CACHE, key = TENANT_KEY + " + ':filter:' + #dto.blocoId() + ':' + #dto.numero()")
    public List<ApartamentoDTO> filterBy(ApartamentoFilterDTO dto) {
        ...
        var result = repository.findAll(Specification.allOf(hasBlocoId(dto.blocoId()), hasNumero(dto.numero())))
                .stream()
     ...
     * @throws org.springframework.web.server.ResponseStatusException if no apartamento exists with the given id
     */
    @Transactional
    ...
    public ApartamentoDTO update(ApartamentoDTO dto) {
        ...
        var entity = repository.findById(dto.id())
                .orElseThrow(() -> notFound("Apartamento", dto.id()));
    ...
    public void delete(Long id) {
        ...
        var entity = repository.findById(id)
                .orElseThrow(() -> notFound("Apartamento", id));
