...
/**
 * Application service for managing blocos (building blocks), with tenant-scoped caching and soft-delete support.
 */
@Slf4j
@Service
...
public class BlocoService {
    ...
    public List<BlocoDTO> filterBy(BlocoFilterDTO dto) {
        ...
        var result = repository.findAll(Specification.allOf(hasBloco(dto.bloco())))
     ...
     * @throws org.springframework.web.server.ResponseStatusException if no bloco exists with the given id
     */
    @Transactional
    ...
    public BlocoDTO update(BlocoDTO dto) {
        ...
        var entity = repository.findById(dto.id())
                .orElseThrow(() -> notFound("Bloco", dto.id()));
