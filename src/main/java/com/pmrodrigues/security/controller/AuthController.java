...
/**
 * REST controller exposing custom login, logout, and token-refresh endpoints outside of the
...
public class AuthController {
  ...
  public ResponseEntity<AuthResponseDTO> login(
      @Valid @RequestBody AuthRequestDTO request, HttpServletRequest httpRequest) {
  ...
  /**
   * Activates a user account using the one-time token sent in the activation email. The account is
   * enabled and the user receives short-lived credentials; the response contains a {@code
   * redirectUrl} pointing to the change-password screen.
   *
  ...
  public ResponseEntity<ActivationResponseDTO> activate(
      @Valid @RequestBody ActivateAccountDTO request) {
  ...
  public ResponseEntity<AuthResponseDTO> refresh(
      @Valid @RequestBody RefreshRequestDTO request, HttpServletRequest httpRequest) {
    rateLimitService.checkRefreshRateLimit(extractClientIp(httpRequest));
    log.info("Token refresh request received");

    var email =
        tokenBlacklistService
            .getEmailByRefreshToken(request.refreshToken())
            .orElseThrow(
                () ->
                    new ResponseStatusException(UNAUTHORIZED, "Invalid or expired refresh token"));

    tokenBlacklistService.deleteRefreshToken(email);
    var newRefreshToken = jwtService.generateRefreshToken();
    tokenBlacklistService.storeRefreshToken(
        email, newRefreshToken, Duration.ofSeconds(jwtProperties.getRefreshTokenExpiration()));
