 ...
 * update is open to all authenticated users but the service enforces owner-only access for
 * non-admins.
 */
@Slf4j
@ApiVersion("1")
...
public class UserController {
   ...
   * @return {@code 201} with the persisted {@link UserDTO}, or {@code 400} if the email is already
   *     in use or the condominium does not exist
   */
  @PostMapping
  @Timed(value = "user.controller.create", description = "Create user")
  @PreAuthorize("hasRole('ADMIN')")
  public ResponseEntity<ApiResponse<UserDTO>> create(
      @RequestBody @Valid CreateUserDTO dto, HttpServletRequest request) {
  ...
  /**
   * Updates an existing user. Admins may update any user; non-admins may only update their own
   * data.
   *
   * @param id the user's primary key, which overrides any {@code id} in the request body
   * @param dto validated replacement user data
   * @return {@code 200} with the updated {@link UserDTO}, or {@code 403} if a non-admin targets
   *     another user
   */
  @PutMapping("/{id}")
  ...
  public ResponseEntity<ApiResponse<UserDTO>> update(
      @PathVariable Long id, @RequestBody @Valid UserDTO dto, HttpServletRequest request) {
