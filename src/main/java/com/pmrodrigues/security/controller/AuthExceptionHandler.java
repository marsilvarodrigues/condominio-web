...
/**
 * Global exception handler for authentication and authorization failures, returning standardized
 * {@link ApiResponse} error payloads and recording metrics.
 */
@Slf4j
...
public class AuthExceptionHandler {
  ...
  private static final String AUTHENTICATION_ERROR_DESCRIPTION =
      "Authentication and authorization error occurrences";

  private final MeterService meterService;

  /**
   * Handles invalid credentials by returning a {@code 401} response with an {@code
   * invalid_credentials} error code.
   *
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleBadCredentials(
    ...
    var response =
        errorResponse(request, UNAUTHORIZED, "invalid_credentials", "Invalid email or password");
    log.info("Returning 401 response for invalid credentials");
    return response;
  }

  /**
   * Handles disabled or locked accounts by returning a {@code 401} response with an {@code
   * account_unavailable} error code.
   *
   * @return {@code 401} with a message indicating whether the account is disabled or locked
   */
  @ExceptionHandler({DisabledException.class, LockedException.class})
  public ResponseEntity<ApiResponse<ErrorResponse>> handleDisabledOrLocked(
  ...
  /**
   * Catch-all handler for any other {@link AuthenticationException}, returning a generic {@code
   * 401} response.
   *
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleAuthentication(
    ...
    var response =
        errorResponse(request, UNAUTHORIZED, "authentication_failed", "Authentication failed");
    log.info("Returning 401 response for authentication failure");
    return response;
  }

  /**
   * Handles {@link ResponseStatusException} by forwarding its status code and reason phrase in the
   * API response.
   *
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleResponseStatus(
    ...
    meterService.incrementError(
        METRIC_NAME, "response_status_error", AUTHENTICATION_ERROR_DESCRIPTION, status);
    var error = new ErrorResponse("error", ex.getReason());
    var response =
        ResponseEntity.status(ex.getStatusCode()).body(ApiResponse.of(requestId(request), error));
    log.info("Returning {} response for response status error", status);
    return response;
  }

  /**
   * Handles bean-validation failures by collecting per-field error messages and returning a {@code
   * 400} response.
   *
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleValidation(
    ...
    log.error(
        "Validation failed with {} field error(s)", ex.getBindingResult().getFieldErrorCount());
    meterService.incrementError(
        METRIC_NAME, "validation_error", AUTHENTICATION_ERROR_DESCRIPTION, BAD_REQUEST.value());
    ...
    var response =
        ResponseEntity.status(BAD_REQUEST).body(ApiResponse.of(requestId(request), error));
  ...
  private ResponseEntity<ApiResponse<ErrorResponse>> errorResponse(
    ...
    meterService.incrementError(
        METRIC_NAME, errorType, AUTHENTICATION_ERROR_DESCRIPTION, status.value());
    return ResponseEntity.status(status)
        .body(ApiResponse.of(requestId(request), new ErrorResponse(errorType, message)));
  }
}
