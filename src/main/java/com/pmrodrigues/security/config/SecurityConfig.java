...
/**
 * Main security configuration for the resource server: stateless JWT authentication, role mapping,
...
public class SecurityConfig {
  ...
  /**
   * Configures the primary security filter chain with stateless sessions, public auth endpoints,
   * and OAuth2 JWT resource server support.
   *
  ...
  /**
   * Maps the {@code roles} claim from the JWT to Spring Security {@code GrantedAuthority} instances
  ...
  /**
   * Creates a {@link DaoAuthenticationProvider} wired with the application's user-details service
   * and BCrypt encoder.
   *
  ...
  /**
   * Exposes the {@link AuthenticationManager} from Spring's auto-configuration for use in
  ...
  /**
   * Restricts CORS to the configured frontend origin only. Credentials are allowed so the browser
