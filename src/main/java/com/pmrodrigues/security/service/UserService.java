 ...
 * Subclasses ({@link com.pmrodrigues.morador.service.PessoaService}, {@link
 * com.pmrodrigues.morador.service.ProprietarioService}) extend this service to reuse the
 * user-creation ceremony via {@link #persistUser}.
 */
@Slf4j
@Service
@Primary
public class UserService {
   ...
   * @param mailService service for sending activation emails
   * @param userMapper MapStruct mapper for DTO conversions
   * @param passwordHistoryRepository repository for tracking password history
   * @param condominioService service for resolving condominium references
  ...
  /**
   * Saves a new user entity, handling email uniqueness, condominium associations, and activation
   * email dispatch. Intended to be called by subclasses in their own {@code create} methods after
   * building the concrete entity.
   *
   ...
   * @throws ResponseStatusException 400 if the email is already in use or a condominium does not
   *     exist
   */
        ...
        .ifPresent(
            existing -> {
              ...
              throw new ResponseStatusException(
                  BAD_REQUEST, "Não foi possível criar o usuário com os dados informados");
            });
  ...
  /**
   * Persists a new user and dispatches an activation email containing the generated temporary
   * password and token.
   *
   ...
   * @throws org.springframework.web.server.ResponseStatusException 400 if the email is already in
   *     use or any condominium does not exist
   */
  @Transactional
   ...
   *
   * <p>Admins may update any user. Non-admins may only update their own data; attempting to update
   * another user's data raises a 403 Forbidden.
   *
   * @return the updated user as a DTO
   * @throws com.pmrodrigues.commons.util.Exceptions if the user does not exist
   * @throws org.springframework.web.server.ResponseStatusException 403 if a non-admin tries to
   *     update another user
   */
  @Transactional
  ...
  public UserDTO update(UserDTO dto) {
    ...
    var user = userRepository.findById(dto.id()).orElseThrow(() -> notFound("User", dto.id()));
  ...
  public void delete(Long id) {
    ...
    var user = userRepository.findById(id).orElseThrow(() -> notFound("User", id));
    userRepository.delete(user);
    log.info("User soft-deleted successfully: {}", id);
  }

  /**
   * Validates the activation token and its expiry, enables the account, and clears the token. The
   * temporary password is kept unchanged; the user must change it after logging in.
   *
   * @param activationToken the one-time token sent to the user's email
   * @return the now-enabled user as a DTO
   * @throws org.springframework.web.server.ResponseStatusException with 404 for an unknown token,
   *     or 400 for an expired token
   */
  @Transactional
  ...
  /**
   * Changes the password for an existing user after verifying the current password, confirming the
   * new password matches its confirmation, and checking that the new password has not been used in
   * the last {@code app.security.password-history-count} changes.
   *
  ...
  public void changePassword(Long userId, ChangePasswordDTO dto) {
    ...
    var user = userRepository.findById(userId).orElseThrow(() -> notFound("User", userId));
    ...
    var history =
        passwordHistoryRepository.findByUserIdOrderByCreatedAtDesc(
            userId, PageRequest.of(0, passwordHistoryCount));
    boolean reused =
        history.stream().anyMatch(h -> passwordEncoder.matches(dto.newPassword(), h.getPassword()));
    if (reused) {
    ...
    passwordHistoryRepository.save(
        PasswordHistory.builder().userId(userId).password(user.getPassword()).build());
  ...
  protected Set<Condominio> loadCondominioEntities(Set<Long> condominioIds) {
    ...
    for (Long condId : condominioIds) {
      var entity =
          condominioService
              .findEntityById(condId)
              .orElseThrow(
                  () -> {
                    log.error("Invalid condominioId: {}", condId);
                    return new ResponseStatusException(
                        BAD_REQUEST, "Condomínio inválido: " + condId);
                  });
      result.add(entity);
    }
    return result;
