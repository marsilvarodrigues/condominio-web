 ...
 *
 * <p>Each instance tracks the full lifecycle of a charge: from creation, through Asaas
 * submission, email dispatch, and final payment or cancellation.
 *
 * <p>Partitioned by {@code condominio_id} (HASH, 4 buckets) for query performance.
 * Soft-deletes are implemented via {@link SQLDelete} + {@link SQLRestriction}.
 */
@Getter
@Setter
@NoArgsConstructor
...
public class Cobranca {
     ...
     * Getter/setter suppressed to prevent bypassing the tenant mechanism.
     */
    ...
    /**
     * Reference to the {@code CotaRateio} that originated this charge.
     * Stored as a plain FK column (not a JPA association) to keep cross-module coupling minimal.
     */
    @Column(name = "cota_rateio_id", nullable = false)
    private Long cotaRateioId;

    /**
     * ID of the {@code Morador} (resident) linked to this charge.
     * May be {@code null} if the apartment has no resident at generation time.
     */
    ...
    /**
     * Sets {@code condominio} from {@link TenantContext} before first persistence,
     * and defaults {@code status} to {@link StatusCobranca#PENDENTE} if not set.
     */
    @PrePersist
