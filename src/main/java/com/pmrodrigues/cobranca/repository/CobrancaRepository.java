 ...
 *
 * <p>Provides standard CRUD, specification-based filtering, and custom lookup methods used by the
 * billing service and webhook handler.
 */
@Repository
public interface CobrancaRepository
  ...
  /**
   * Finds a charge by its originating CotaRateio ID. Used for idempotency checks during charge
   * generation.
   *
  ...
  /**
   * Returns all charges with the given status where the billing email has not been sent. Used by
   * scheduled rematch / resend jobs.
   *
  ...
  /**
   * Looks up a charge by its Asaas payment ID using a native query. Bypasses Hibernate's tenant
   * filter (the WHERE clause includes {@code deleted = false}).
   *
   * @param asaasId Asaas payment ID
   * @return optional charge, empty if not found
   */
  @Query(
      value = "SELECT * FROM cobrancas WHERE asaas_id = :asaasId AND deleted = false LIMIT 1",
      nativeQuery = true)
   ...
   * CondominioService.delete()} to maintain cascade integrity.
   *
