 ...
 *
 * <p>All mutating operations (charge generation, cancellation, email resend) require the
 * {@code ADMIN} role. Read operations are accessible to any authenticated user.
 * The Asaas webhook endpoint is unauthenticated — it validates the {@code access_token}
 * request header against the configured API key.
 */
@Slf4j
@ApiVersion("1")
@RestController
...
public class CobrancaController {
     ...
     */
    @PostMapping("/gerar")
    @Timed(value = "cobranca.controller.gerar", description = "Generate charges for a rateio execution")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<CobrancaDTO>>> gerar(
            @RequestBody @Valid GerarCobrancasDTO dto,
    ...
    public ResponseEntity<ApiResponse<Page<CobrancaDTO>>> filterBy(
        ...
        return ResponseEntity.ok(ApiResponse.of(requestId(request),
                cobrancaService.filterBy(filter, pageable)));
    }
    ...
    public ResponseEntity<ApiResponse<CobrancaDTO>> cancelar(
        ...
        return ResponseEntity.ok(ApiResponse.of(requestId(request),
                cobrancaService.cancelar(id, dto)));
    }
    ...
    public ResponseEntity<ApiResponse<Page<CobrancaResumoDTO>>> porApartamento(
        ...
        return ResponseEntity.ok(ApiResponse.of(requestId(request),
                cobrancaService.porApartamento(apartamentoId, pageable)));
    }

    /**
     * Asaas payment webhook endpoint — no JWT authentication required.
     *
     * <p>Validates the incoming {@code access_token} header against the configured API key
     * before processing. Returns 403 if the token does not match.
     *
