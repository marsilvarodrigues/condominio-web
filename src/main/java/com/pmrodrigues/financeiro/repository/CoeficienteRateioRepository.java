...
public interface CoeficienteRateioRepository extends JpaRepository<CoeficienteRateio, Long> {
    ...
    /**
     * Returns the most recent active coefficient records for each apartment in the group,
     * using the record whose {@code vigencia} is on or before the reference date.
     *
    ...
    List<CoeficienteRateio> findVigentesPorGrupo(
            @Param("grupoDespesaId") Long grupoDespesaId,
