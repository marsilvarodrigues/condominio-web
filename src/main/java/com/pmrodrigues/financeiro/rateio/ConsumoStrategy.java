...
/**
 * Splits the expense into a fixed portion (equally divided) and a variable portion (by
 * consumption).
 *
 * <p>Formula:
 *
 ...
 * </pre>
 *
 * <p>If {@code Σconsumos == 0}, the variable portion is split equally and a warning is logged. The
 * rounding remainder is assigned to the unit with the highest consumption.
 *
...
public class ConsumoStrategy implements EstrategiaRateio {
  ...
  public Map<Long, BigDecimal> calcular(
    ...
    Map<Long, BigDecimal> cotasVariaveis =
        calcularVariavel(ordenados, despVariavel, somaConsumos, consumoZero);
  ...
  private BigDecimal extrairPercentualFixo(Map<String, Object> parametros) {
    ...
    if (valor == null) {
      throw new IllegalArgumentException("Parâmetro obrigatório ausente: " + PARAM_PERCENTUAL_FIXO);
    }
    BigDecimal pf;
