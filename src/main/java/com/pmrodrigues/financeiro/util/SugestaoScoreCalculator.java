...
/**
 * Pure-function utility that scores how well an {@link ItemOrcamento} matches
 * a given {@link ItemExtrato}. Score is 0–100 (higher is better).
 *
 * <p>Scoring breakdown:
 * <ul>
 *   <li>0–50 points — keyword overlap between statement description and account name</li>
 *   <li>0–30 points — CREDITO↔RECEITA / DEBITO↔DESPESA type compatibility</li>
 *   <li>0–20 points — value proximity (full 20 pts when values match; 0 when ≥100 % apart)</li>
 * </ul>
 */
public final class SugestaoScoreCalculator {
    ...
    public static int calcularScore(ItemExtrato extrato, ItemOrcamento orcamento) {
        int score = keywordScore(extrato.getDescricao(), orcamento.getPlanoContas().getDescricao())
    ...
    public static boolean tipoCompativel(TipoLancamento tipo, TipoConta tipoConta) {
        if (tipo == null || tipoConta == null) return false;
        return (tipo == TipoLancamento.CREDITO && tipoConta == TipoConta.RECEITA)
                || (tipo == TipoLancamento.DEBITO && tipoConta == TipoConta.DESPESA);
    }

    /**
     * Returns a human-readable explanation of why {@code orcamento} was suggested for {@code extrato}.
     */
    ...
    public static int keywordScore(String descricaoExtrato, String descricaoOrcamento) {
        if (descricaoExtrato == null || descricaoOrcamento == null) return 0;
    ...
    public static int valorScore(BigDecimal valorExtrato, BigDecimal valorPrevisto) {
        if (valorExtrato == null || valorPrevisto == null
                || valorPrevisto.compareTo(BigDecimal.ZERO) == 0) return 0;
        BigDecimal diff = valorExtrato.subtract(valorPrevisto).abs();
        BigDecimal ratio = diff.divide(valorPrevisto, 4, RoundingMode.HALF_UP);
        if (ratio.compareTo(BigDecimal.ONE) >= 0) return 0;
