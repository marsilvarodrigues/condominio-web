 ...
 *
 * <p>Abstract class required because {@code paiFromId} uses
 * {@code EntityManager.getReference()} to obtain a JPA proxy for the parent node,
 * which cannot be injected via the standard MapStruct {@code uses} mechanism.
 * Setter injection with {@code required = false} prevents injection failures
 * in cache-only test contexts where no EntityManager bean is present.
 */
