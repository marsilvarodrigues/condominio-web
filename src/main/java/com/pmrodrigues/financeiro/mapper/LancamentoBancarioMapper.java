...
public interface LancamentoBancarioMapper {
  ...
  /**
   * Converts a {@link LancamentoBancario} entity to its DTO, flattening the conta bancaria
   * reference.
   */
  ...
  /**
   * Creates a new {@link LancamentoBancario} from a creation payload. The {@code contaBancaria} and
  ...
  /**
   * Updates mutable fields of an existing {@link LancamentoBancario}. Tipo, origem, contaBancaria,
   * and status cannot be changed via update.
   */
  @Mapping(target = "id", ignore = true)
  @Mapping(target = "contaBancaria", ignore = true)
