...
import static com.pmrodrigues.financeiro.specification.ContaBancariaSpecification.*;

/**
 * Service managing bank accounts for the current condominium tenant.
 * Enforces the single-FUNDO_RESERVA-per-condominium constraint at the service layer.
 */
@Slf4j
@Service
...
public class ContaBancariaService {
    ...
    public ContaBancariaDTO findById(Long id) {
        ...
        var result = repository.findById(id)
                .map(mapper::toDTO)
                .orElseThrow(() -> notFound("ContaBancaria", id));
        log.info("ContaBancaria lookup by id {}: found", id);
        return result;
    }

    /**
     * Creates a new bank account for the current condominium.
     * Refuses if a FUNDO_RESERVA account already exists and the new type is also FUNDO_RESERVA.
     *
     ...
     */
    @Transactional
    @Timed(value = "conta.bancaria.service.softDeleteByCondominioId", description = "Cascade soft-delete contas bancarias by condominio")
    @CacheEvict(value = CACHE, allEntries = true)
