...
/**
 * Application service managing annual budgets through their lifecycle, including item management and approval.
 */
@Slf4j
@Service
...
public class OrcamentoAnualService {
    ...
    @Cacheable(value = CACHE, key = TENANT_KEY + " + ':filter:' + #dto.exercicio() + ':' + #dto.status()")
    public List<OrcamentoAnualDTO> filterBy(OrcamentoAnualFilterDTO dto) {
        ...
        var result = repository.findAll(Specification.allOf(hasExercicio(dto.exercicio()), hasStatus(dto.status())))
    ...
    public OrcamentoAnualDTO aprovar(Long id, AprovarOrcamentoDTO dto) {
        ...
        var totalPrevisto = items.stream()
                .map(ItemOrcamento::getValorPrevisto)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        var taxa = totalPrevisto.divide(
    ...
    public OrcamentoAnualDTO encerrar(Long id) {
        ...
        if (entity.getStatus() != StatusOrcamento.APROVADO) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Apenas orçamentos APROVADOS podem ser encerrados");
        }
