...
public class CoeficienteRateioService {
  ...
  @Timed(
      value = "coeficiente.rateio.service.findByGrupo",
  ...
  public List<CoeficienteRateioDTO> findByGrupo(Long grupoDespesaId) {
    ...
    return repository.findByGrupoDespesaIdOrderByVigenciaDesc(grupoDespesaId).stream()
        .map(mapper::toDTO)
        .toList();
  }
  ...
  public CoeficienteRateioDTO create(Long grupoDespesaId, CreateCoeficienteRateioDTO dto) {
    log.info(
        "Creating coeficiente for grupoDespesaId={}, apartamentoId={}",
        grupoDespesaId,
        dto.apartamentoId());
    var grupo =
        grupoDespesaService
            .findEntityById(grupoDespesaId)
            .orElseThrow(() -> notFound("GrupoDespesa", grupoDespesaId));
    var apartamento =
        apartamentoService
            .findEntityById(dto.apartamentoId())
  ...
  public CoeficienteRateioDTO updateConsumo(Long coefId, UpdateConsumoDTO dto) {
    ...
    var entity =
        repository.findById(coefId).orElseThrow(() -> notFound("CoeficienteRateio", coefId));
