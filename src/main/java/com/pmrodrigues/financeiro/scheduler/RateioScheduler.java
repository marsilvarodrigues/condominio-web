 ...
 *
 * <p>Runs every day at 02:00 São Paulo time. Each condominium is processed independently: a failure
 * in one tenant does not affect the others. The {@link TenantContext} is always cleared in a {@code
 * finally} block to prevent tenant leakage across iterations.
 */
@Slf4j
@Component
...
public class RateioScheduler {
  ...
  /**
   * Processes all pending and failed despesas for every active condominium. Scheduled daily at
   * 02:00 São Paulo time (America/Sao_Paulo).
   */
  ...
  private void processarCondominio(Long condominioId) {
    ...
    try {
      ...
      log.info(
          "Rateio automático — condominioId={} total={} sucesso={} erro={} duracaoMs={}",
          condominioId,
          resultado.total(),
          resultado.sucesso(),
          resultado.erro(),
      ...
      log.error(
          "Falha no rateio automático — condominioId={}: {}", condominioId, e.getMessage(), e);
    } finally {
      TenantContext.clear();
    }
  }
