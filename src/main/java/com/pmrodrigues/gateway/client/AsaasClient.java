 ...
 *
 * <p>All methods call {@code .block()} because they are invoked from virtual-thread worker contexts
 * — blocking is safe and avoids reactive context propagation overhead.
 *
 * <p>The {@code access_token} header is injected on every request via a default header configured
...
public class AsaasClient {
                        ...
                        .map(
                            body ->
                                new ResponseStatusException(
                                    HttpStatus.BAD_GATEWAY, "Asaas error: " + body)))
  ...
  public AsaasCobrancaResponse criarCobranca(AsaasCobrancaRequest request) {
    log.info(
        "Creating Asaas charge: customer={} value={} dueDate={}",
        request.customer(),
        request.value(),
        request.dueDate());
    return webClient
  ...
  public void cancelarCobranca(String asaasId) {
    ...
    webClient
        ...
        .onStatus(
            ...
            resp ->
                resp.bodyToMono(String.class)
                    .map(body -> new ResponseStatusException(HttpStatus.BAD_REQUEST, body)))
        .onStatus(
            HttpStatusCode::is5xxServerError,
            resp ->
                resp.bodyToMono(String.class)
