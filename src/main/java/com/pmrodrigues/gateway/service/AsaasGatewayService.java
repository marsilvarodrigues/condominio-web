...
import com.pmrodrigues.gateway.dto.*;
 ...
 *
 * <p>Orchestrates customer resolution, charge creation, and Pix QR Code generation into
 * a single operation ({@link #emitir}), shielding callers from the multi-step Asaas API flow.
 */
@Slf4j
@Service
...
public class AsaasGatewayService {
     ...
     * already exists for the given {@code pessoaId} it is reused; otherwise a new Asaas customer
     * is created and the mapping is persisted.
     *
    ...
    public AsaasEmissaoResult emitir(AsaasEmissaoRequest req) {
        log.info("emitir pessoaId={} valor={} vencimento={}", req.pessoaId(), req.valor(), req.vencimento());
    ...
    private String resolverCustomerId(Long pessoaId, String nome, String cpf, String email) {
        return asaasCustomerRepository.findByPessoaId(pessoaId)
                .map(AsaasCustomer::getCustomerId)
                .orElseGet(() -> {
                    String customerId = asaasClient.criarCustomer(
                            new AsaasCustomerRequest(nome, cpf, email, null));
                    asaasCustomerRepository.save(AsaasCustomer.builder()
                            .pessoaId(pessoaId)
